...
- **Evidence leaves the guest as it happens.** The transcript is the harness's own event stream, written to a host file as the guest emits it; `completed` in `metrics.json` says whether its last event is a success result. The working tree and the guest console come out the same way. Model-written code never runs on the host.
- **Checked in a fresh VM.** After the turn, a second VM that receives only the tree runs `cargo test` and `cargo clippy` as an unprivileged user; their exit codes reach the host on a port only that VM's root can write.
...
A finished run lands in `runs/<test>/<model>/<UTC timestamp>/` with `run.json`, `transcript.jsonl`, `tree.tar.zst` (the working tree without build outputs), `check.log` (the test/clippy pass), `console.log` (both VMs) and `metrics.json`; a summary adds `summary.md`. A run whose artifacts contain the login is discarded. In `metrics.json`, `tokens.input` counts every input token, cache reads and writes included, and `wall_clock_s` spans the agent's VM from launch to exit. `bin/metrics <run-dir>` recomputes it.
